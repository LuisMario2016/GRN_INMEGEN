...
get_sets=function(enriched_table,exclusive){
  ...
  # print the content of g for debugging
  print(g)
...
png("BPexclusive.png")
BP.classes%>%dplyr::count(subtype,name)%>%
  ...
  annotate("text",x=1.08,y=sort(unique(BP.classes$name)),
           label=BP.classes%>%dplyr::count(name)%>%dplyr::select(n)%>%unlist)+
